...
| Grid-charge floor-protection gate | Done | `optimizer-lp.js`: `config.battery.grid_charge_floor_buffer_soc` (default 5; `null` disables). Margin SOC = `min_soc + buffer` (currently 8 + 5 = 13%). Before building the LP bounds, simulates a **baseline no-grid-charge SOC trajectory** (solar surplus charges, consumption deficit discharges, no bottom clamp). **The gate only restricts the pre-sunrise (night) window** — daylight slots (`solar_watts ≥ MIN_SOLAR_W`) stay **open** so the LP decides on normal economics (free solar on sunny days; cheap grid for an expensive evening on overcast days with no surplus). For each pre-sunrise slot, if the battery can bridge to the next **solar onset (sunrise)** while staying ≥ the margin, the slot's `cg_t` upper bound is **zeroed** — no grid charge. Grid charging pre-sunrise is permitted only where a margin breach is otherwise unavoidable before sunrise. Eliminates pre-dawn arbitrage/hedge buying (e.g. 2026-06-08 05:00 charge under a pessimistic cloudy forecast) while still allowing cheap daytime charging to cope with the night on near-zero-solar days. Robust to solar under-forecast because the pre-sunrise bridge carries ~zero solar. Independent of (and stacks with) `charge_grid_max_buy_price`. |
...
`cg_t`'s upper bound is additionally zeroed by two independent gates: the `charge_grid_max_buy_price` price ceiling and the `grid_charge_floor_soc` floor-protection gate (see the feature table) — the latter confines grid charging to slots where the battery cannot otherwise bridge to sunrise above the safety margin.
