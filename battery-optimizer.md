...
| Grid-charge floor-protection gate | Done | `optimizer-lp.js`: `config.battery.grid_charge_floor_buffer_soc` (default 5; `null` disables). Margin SOC = `min_soc + buffer` (currently 8 + 5 = 13%). Before building the LP bounds, simulates a **baseline no-grid-charge SOC trajectory** (solar surplus charges, consumption deficit discharges, no bottom clamp). For each slot, if the battery can bridge to the next **solar onset** (`solar_watts ≥ MIN_SOLAR_W` — sunrise) while staying ≥ the margin, the slot's `cg_t` upper bound is **zeroed** — no grid charge. Grid charging is permitted only where a margin breach is otherwise unavoidable before sunrise (genuine floor protection). Eliminates pre-dawn arbitrage/hedge buying (e.g. 2026-06-08 05:00 charge under a pessimistic cloudy solar forecast). Robust to solar under-forecast because the pre-solar bridge carries ~zero solar — on a genuinely cloudy day the battery rides toward the margin and imports directly rather than pre-buying. Independent of (and stacks with) `charge_grid_max_buy_price`. |
