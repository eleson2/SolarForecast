...
## Clarification: charge_grid decisions at comfortable SOC are INTENTIONAL price arbitrage — floor-protection gate never restricts daylight slots (confirmed 2026-07-12)
- **What**: `grid_charge_floor_buffer_soc` (config.js battery, default 5, margin = min_soc+buffer = 14+5=19%) only gates grid-charging in PRE-SUNRISE slots. In `src/optimizer-lp.js` line 449 — `if (isRelief[t]) { cgGateOpen[t] = true; continue; }` — any slot with `solar_watts >= MIN_SOLAR_W` (50W, line 143) is a "relief" slot and the gate is unconditionally OPEN, regardless of current SOC. So during the day the LP is always free to grid-charge on pure economics; the floor-protection gate (lines 423-464) exists solely to stop wasteful *pre-dawn* hedge-buying, not to require "need" during daylight.
- **Why this is correct behavior, not a bug**: the objective (lines 294-330) purely minimizes Σ buy_price·cg_t − Σ buy_price·d_t over the full rolling window (`optimizer_window_hours: 48`). When the day's price spread is large (confirmed 2026-07-12: 0.0326 SEK/kWh at 09:45 vs 0.94 SEK/kWh at 21:00 — a ~23–29× spread, dwarfing the ~11% round-trip efficiency loss), it is always cheaper to buy now and discharge later than to import directly during the evening peak, irrespective of whether current SOC (e.g. 38-40%, ~20pts above min_soc=14%) is "comfortable." This is textbook arbitrage and matches the LP's own logged savings estimate that day (`LP optimizer: savings 6.29 SEK`).
- **Also supported by forecast context**: on 2026-07-12 the solar forecast at optimize-time showed heavy cloud cover (88-100%) and low expected hourly production (e.g. 0.28-0.42 kWh/hr forecast for 09:00-10:00) — actual production later outperformed forecast (known correction-matrix-still-learning pattern), but at decision time the LP had reason to expect a weak solar day and to secure cheap energy rather than count on free midday surplus.
- **How to audit this going forward**: when a user flags a charge_grid slot as "unnecessary" because SOC was high, check (a) the day's price spread (min vs peak-hour price) via `battery_schedule.price_kwh`, and (b) whether the slot is a daylight slot (`solar_watts >= 50W` at that slot) — if so, the floor gate does NOT apply and the decision was pure arbitrage, not a bug. Only pre-sunrise charge_grid slots are gated by SOC margin; see the "Dawn pre-charge" entries below for that failure mode.
- **Separate issue still stands**: the *power magnitude* of execution (see entry above) is uncapped regardless of whether the underlying charge decision was rational — a rational "top up 0.8 kWh" decision can still execute as an irrational "charge 2 kWh at 8kW" dispatch due to the SOC-floor-only hardware control.
